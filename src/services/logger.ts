// src/services/logger.ts — Server-side logger (mirrors shared/logger.ts)
// This is a copy of shared/logger.ts because the server Docker build compiles
// only src/ and does not include shared/ in the runtime image.
// Keep in sync with shared/logger.ts.
import {
  appendFileSync,
  writeFileSync,
  existsSync,
