/**
 * Storage factory: creates repository instances based on CONFIG.storageBackend.
 *
 * When `CONFIG.storageBackend === "postgres"`, returns lazy Postgres repository
 * proxies that initialise the client and run migrations on first use.
 * Otherwise returns lazy SQLite implementations.
 *
 * The laziness avoids importing heavy singletons until after config
 * initialization, and ensures importing the factory in SQLite mode does not
 * connect to Postgres.
 */
...
// ── Postgres lazy proxy helpers ──
// These mirrors the SQLite lazy-proxy pattern but import from postgres/ modules.

class LazyPostgresMemoryRepository implements MemoryRepository {
...
class LazyPostgresUserPromptRepository implements UserPromptRepository {
...
class LazyPostgresUserProfileRepository implements UserProfileRepository {
  ...
  mergeProfileData(existing: UserProfileData, updates: Partial<UserProfileData>): UserProfileData {
    // Synchronous method — use the same pure logic as the SQLite factory.
    // The Postgres implementation's mergeProfileData is also pure/synchronous,
    // but since the repo is lazily loaded (async), we inline the same logic here.
...
class LazyPostgresAISessionRepository implements AISessionRepository {
...
class LazySqliteMemoryRepository implements MemoryRepository {
  private target: Promise<MemoryRepository> | null = null;

  private async repo(): Promise<MemoryRepository> {
    if (!this.target) {
      this.target = import("./sqlite-memory-repository.js").then(
        ({ SqliteMemoryRepository }) => new SqliteMemoryRepository()
      );
    }
    return this.target;
  }
...
class LazySqliteUserPromptRepository implements UserPromptRepository {
  private target: Promise<UserPromptRepository> | null = null;

  private async repo(): Promise<UserPromptRepository> {
    if (!this.target) {
      this.target = import("./sqlite-user-prompt-repository.js").then(
        ({ SqliteUserPromptRepository }) => new SqliteUserPromptRepository()
      );
    }
    return this.target;
  }
...
class LazySqliteUserProfileRepository implements UserProfileRepository {
  private target: Promise<UserProfileRepository> | null = null;

  private async repo(): Promise<UserProfileRepository> {
    if (!this.target) {
      this.target = import("./sqlite-user-profile-repository.js").then(
        ({ SqliteUserProfileRepository }) => new SqliteUserProfileRepository()
      );
    }
    return this.target;
  }
...
class LazySqliteAISessionRepository implements AISessionRepository {
  private target: Promise<AISessionRepository> | null = null;

  private async repo(): Promise<AISessionRepository> {
    if (!this.target) {
      this.target = import("./sqlite-ai-session-repository.js").then(
        ({ SqliteAISessionRepository }) => new SqliteAISessionRepository()
      );
    }
    return this.target;
  }
...
export function createMemoryRepository(): MemoryRepository {
  ...
  if (CONFIG.storageBackend === "postgres") {
    memoryRepo = new LazyPostgresMemoryRepository();
  } else {
    memoryRepo = new LazySqliteMemoryRepository();
  }
  return memoryRepo;
}
...
export function createUserPromptRepository(): UserPromptRepository {
  ...
  if (CONFIG.storageBackend === "postgres") {
    promptRepo = new LazyPostgresUserPromptRepository();
  } else {
    promptRepo = new LazySqliteUserPromptRepository();
  }
  return promptRepo;
}
...
export function createUserProfileRepository(): UserProfileRepository {
  ...
  if (CONFIG.storageBackend === "postgres") {
    profileRepo = new LazyPostgresUserProfileRepository();
  } else {
    profileRepo = new LazySqliteUserProfileRepository();
  }
  return profileRepo;
}
...
export function createAISessionRepository(): AISessionRepository {
  ...
  if (CONFIG.storageBackend === "postgres") {
    sessionRepo = new LazyPostgresAISessionRepository();
  } else {
    sessionRepo = new LazySqliteAISessionRepository();
  }
  return sessionRepo;
}
 ...
 * For SQLite this checkpoints all WAL files; for Postgres this is a no-op
 * (server-side WAL handles durability).
 */
export async function checkpointStorage(): Promise<void> {
